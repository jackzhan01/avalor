...
**关键在于这两层是同一件事的两面。** 市面上你也可以打开任何一个聊天机器人，然后对它复述「9 人局第 4 轮，我觉得 3 号有问题」—— 它只能陪你聊天，因为它手上什么都没有。Avalor 的 AI 读到的是**每一辆车、每一张座位级的票、意向车和真发车的差异、每一次改口的完整链条、谁从头到尾没表过态**。

**记得越细，它说得越准。** 记录不是目的，是燃料。
...
不是一段和稀泥的分析，是一张表：每个座位一个判断、一句落到具体车和票上的依据、外加它自己的把握程度（**把握大 / 一般 / 只是猜**）。最后给 3–5 条关键结论和一条「最大的风险在哪」。

**它按你的身份分岔**，因为不同身份要回答的根本不是同一个问题：
...
给梅林做一份「谁是坏人」的分析是没有意义的 —— 他早就看见了。他真正睡不着觉的问题是「我刚才那句话是不是说得太准了」。这一条，通用聊天机器人不会替你想到。

### 「帮我发言」—— 给大纲，不给讲稿

一轮发言只有一分钟，话得你自己说。所以它给的是**按先后顺序排好的 4–6 条要点**、一句总基调，以及最要命的那一栏：**这轮千万别说的话**（梅林不能锤得太准，坏人别把队友卖了）。想定方向就塞一句「我想锤 4 号」，它照着这个基调排。满意就一键存进草稿。

### 它凭什么比你自己开个聊天窗口强

送进模型的不是一坨原始 JSON，而是 `briefing.ts` 这个**纯函数**把整份事件日志渲染成的一篇中文局势简报。该分开的地方绝不合并：

- 「没表过态」和「明确说了中立」分两行写，绝不并成一句
- 意向车和真发的车分两段列，所以「说带 1/3/5、真带 2/4/6」这个差异送得到模型手上
- 女神当众说的话和你实际看到的结果分开放 —— 前者可能是谎
- 改过口的地方，整条链子都在，不是只给最新那个数

提示词里则写死了模型最容易搞错的规则（第 4 轮双坏票、莫德雷德不被梅林看见、只有上车的人能投任务牌），以及该怎么推：**票型比嘴硬**、失败任务的上车名单取交集、看一个人改口的**时间点**比看他改成了几分更重要。

还有一条同样重要：**它被明确要求承认不知道。**「信息不够就说不好说，编一个笃定的结论比说不知道更糟」—— 这句话就写在提示词里。座位判断是自由文本而不是固定枚举，因为「可能是坏人，但也可能是被做局的好人」这种话塞进任何一个枚举，丢掉的恰恰是最有用的那半句。
